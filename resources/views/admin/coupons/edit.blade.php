            ...
            <form class="form-new-product form-style-1" method="POST" action="{{ route('coupon.update', $coupon->id) }}">
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Coupon Code" name="code"
                        tabindex="0" value="{{ old('code', $coupon->code) }}" aria-required="true" >
                        @error('code')
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Coupon Value" name="value"
                        tabindex="0" value="{{ old('value', $coupon->value) }}" aria-required="true" >
                </fieldset>
                <fieldset class="name">
                    <div class="body-title">Cart Value <span class="tf-color-1">*</span></div>
                    <input class="flex-grow" type="text" placeholder="Cart Value"
                        name="cart_value" tabindex="0" value="{{ old('cart_value', $coupon->cart_value) }}"
                        >
                        @error('cart_value')
                            <span class="text-danger text-center h6">{{ $message }}</span>
                        @enderror
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="date" placeholder="Expiry Date"
                        name="expiry_date" tabindex="0" value="{{ old('expiry_date', $coupon->expiry_date) }}" >
                        @error('expiry_date')
                            <span class="text-danger text-center h6">{{ $message }}</span>
                        @enderror
                </fieldset>
